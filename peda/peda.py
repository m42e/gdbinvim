...
sys.path = [os.path.dirname(PEDAFILE) + "/lib/"] + sys.path
...
class PEDACmd(object):
    ...
    #################################
    #   Specific commands for vim   #
    #   data written in a fifo      #
    #################################

    def _write_to_fifo(self, data):
        fd = open("result_fifo", "w")
        fd.write(data)
        fd.close()


    def _vimdisassemble(self, start):
        breakpoints = peda.get_breakpoints()
        bpaddr = []
        res = ""
        for b in breakpoints:
            bpaddr += [b[4]]

        bpaddr = list(set(bpaddr))

        for b in bpaddr:
            res += "%d " % b
        res += "\n"

        dump = gdb.execute("disassemble " + start + "," + start + "+100", 
                            to_string=True)

        for line in dump.split("\n")[1:-2]:
            idx = line.find(":")
            res += line[0:idx+1] + (" " * (30 - idx)) + line[idx+1:] + "\n"
            
        self._write_to_fifo(res)
        return


    def vimdeletebreakpoint(self, addr):
        """
        vimdeletebreakpoint
        Usage:
            MYNAME address
        ...
        try:
            ...
            for b in breakpoints:
                if b[4] == addr:
                    gdb.execute("delete " + b[0])

            self._write_to_fifo("###") # for synchro
            
        except Exception as e:
            self._write_to_fifo(str(e))
        return


    def vimdisassemble(self):
        """
        vimdisassemble : disassemble at $pc
        first line contains all breakpoints
        Usage:
            MYNAME
        """
        try:
            self._vimdisassemble("$pc")
        except Exception as e:
            self._write_to_fifo(str(e))
        return


    def vimdisassembleat(self, start):
        """
        vimdisassembleat
        first line contains all breakpoints
        Usage:
        ...
        """
        try:
            self._vimdisassemble(start)
        except Exception as e:
            self._write_to_fifo(str(e))
        return


    def viminfotarget(self):
        """
        viminfotarget
        Usage:
            MYNAME command
        """
        try:
            dump = gdb.execute("info target", to_string=True).split("\n")
            self._write_to_fifo(dump[0].replace("Symbols from ", "")[1:-2])
        except Exception as e:
            self._write_to_fifo(str(e))
        return


    def vimget(self, command):
        """
        vimget : get command result from vim
        Usage:
            MYNAME command
        """
        ...
        try:
            self._write_to_fifo(re.sub("\x1b\\[(;{0,1}[0-9]*)*m", "", 
                        gdb.execute(command, to_string=True)))
        except Exception as e:
            self._write_to_fifo(str(e)) # for synchro
    ...
    def vimexecsilent(self, command):
        """
        vimexecsilent : exec command from vim, but result printed in gdb
        output not printed and not returned to vim
        Usage:
            MYNAME command
        """
    ...
    def vim_disable_hook_stop(self):
        """
        vim_disable_hook_stop
        Usage:
            MYNAME
        """
        peda.save_user_command("hook-stop")
    ...
    def vim_enable_hook_stop(self):
        """
        vim_enable_hook_stop
        Usage:
            MYNAME
        """
        peda.restore_user_command("hook-stop")
