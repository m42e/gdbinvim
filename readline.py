...
class ReadLine():
    ...
    def restore_history(self):
        try:
            fd = open(".gdb_history", "r")
            for cmd in fd:
                self.history = [cmd[:-1]] + self.history
            fd.close()
        except:
            pass
    ...
    def loop(self):
        ...
        while 1:
            ...
            ch = os.read(self.tty_fd, 8) # don't know the maximum of chars to read
    ...
    def print_prompt(self):
        os.write(1, "\x1b[" + str(self.cursor_i) + ";1H")
        os.write(1, yellow(self.prompt))
    ...
    def get_position(self):
        ...
        pos = [0,0]
        try:
            os.write(self.tty_fd, "\x1b[6n")
            ch = os.read(self.tty_fd, 16) # don't know the maximum of chars to read
            pos = ch[2:-1].split(";")
            pos[0] = int(pos[0])
            pos[1] = int(pos[1])
        finally:
